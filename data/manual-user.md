...
- 権限が `viewer` の場合は閲覧専用となり、編集操作は保存できません。
- ユーザー情報の変更は管理者が「マスタ管理 > ユーザー管理」から行います。
...
- 品目は検索可能な入力欄から選択し、数量・メモを入力して保存してください。
- 製造日と賞味期限（製造日＋賞味期限日数）はモーダル内で自動表示されます。
- 既存ブロックはドラッグで移動、左右端をドラッグすると長さを変更できます（承認済みブロックは移動・長さ変更不可）。
