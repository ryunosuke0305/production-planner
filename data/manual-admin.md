...
- ブロックには `laneRow`（サブレーン行）を保持します。
- 品目コード（`publicId` / 品目ID）は必須です。
- 品目コード（品目ID）に加え、計画方針/安全在庫/自動計算設定/賞味期限/製造・包装効率/備考などの品目マスタ情報も SQLite に保存されます。
...
### 権限・アクセスの運用
- マスタ管理は管理者が担当し、依頼者は未承認ブロックと日別在庫の取り込みのみを担当する運用にしてください。
- マスタ管理のトップ画面から「品目一覧」「原料一覧」を開き、登録内容を更新します。
- 品目一覧では安全在庫の自動計算（直近N日出荷 × 係数）、賞味期限、製造効率、包装効率、備考などの項目を合わせて更新します。
- 単位は「ピース / ケース / セット / kg / 袋 / 枚 / 個 / 箱」から選択します。製造効率・包装効率は品目単位/人時で確認できます。
- レシピ設定では原料を検索可能な入力欄から選択できます。
- 承認済みブロックはドラッグ移動や長さ変更ができないため、変更が必要な場合は承認状態を解除してください。
- 計画編集モーダルには登録者・更新者の情報が表示されます。
- 品目・原料マスタの削除時は、確認メッセージで実行可否を判断してから削除します。
- 共有端末ではブラウザの自動入力やパスワード保存を無効にすることを推奨します。

### 在庫データの確認
- メニューの「在庫データ」で、取り込まれている日別在庫と出荷数を一覧で確認できます。
- メニューの「計画一覧」で、登録済みブロックをテーブル表示し、検索条件（キーワード/承認状態）で絞り込みできます。
- 計画一覧テーブルは横/縦スクロール対応で、列幅はドラッグで手動調整・ダブルクリックで自動幅へ戻せます。
- 計画一覧の左端には固定の「編集」列があり、対象行から直接計画編集ダイアログを開けます。
- 品目名の横に単位を表示し、日付ごとのセルは在庫数と出荷数を数値中心で確認できます。
...
### 計画データが表示されない場合
- `data/plan.sqlite` はサーバー起動時に自動作成されます。作成されない場合は、`data/` ディレクトリの書き込み権限を確認します。
