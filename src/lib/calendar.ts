...
import { addDays, diffDays, getWeekdayIndexInTimeZone, parseISODateJST, toISODate } from "@/lib/datetime";

export function buildCalendarDays(start: Date, days: number): CalendarDay[] {
  const baseISO = toISODate(start);
  const baseDate = parseISODateJST(baseISO) ?? start;
  const out: CalendarDay[] = [];
  for (let i = 0; i < days; i += 1) {
    const d = addDays(baseDate, i);
    const isoDate = toISODate(d);
    const weekday = getWeekdayIndexInTimeZone(d);
    out.push({
      date: isoDate,
...
export function extendCalendarDaysTo(calendarDays: CalendarDay[], targetEndISO: string): CalendarDay[] {
  ...
  const appendBase = parseISODateJST(lastISO) ?? new Date(lastISO);
  const appendStart = addDays(appendBase, 1);
