...
import type { Block, Item } from "@/types/planning";
...
export function PlanListView({ blocks, items, slotIndexToLabel, canEdit, onEdit }: PlanListViewProps): JSX.Element {
              ...
              <thead className="sticky top-0 z-30 bg-white">
                <tr>
                  {[
                    ...
                    return (
                      <th key={key} className={`relative border-b px-3 py-2 text-left font-medium ${stickyClass}`}>
                ...
                {filteredRows.length === 0 ? (
                  ...
                  filteredRows.map((row) => (
                    <tr key={row.block.id} className="even:bg-muted/20">
                      ...
                      <td className="border-b px-3 py-2 text-right">{formatQuantity(row.block.amount)}</td>
