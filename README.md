...
- ブロックの追加、移動（上下に日をまたいで移動可能）、リサイズ（0.5スロット単位で調整可能）、メモ編集（承認済みブロックは移動・リサイズ不可）
- ブロック移動は勤務時間内に制限（ドラッグ開始位置を維持）
- 同一日・同一時間帯への重なり配置を許可し、重なり時はサブレーン（laneRow）で縦積み表示
- 原料・品目マスタの編集（マスタ管理の子ページで一覧を表示）
- 品目/原料の選択は検索可能な入力欄で絞り込み
- 計画ブロック編集ダイアログの品目入力欄にクリア（×）アイコンを表示
- 計画ブロック編集ダイアログの下部アクション（キャンセル / 削除 / 保存）を固定表示
- マスタ/計画ブロックの削除時に確認メッセージを表示
- 日次在庫と原料消費量の計算表示（各日付の在庫数を基準に、当日以降の製造計画を反映）
- 取り込み済み日別在庫の一覧表示（品目×日付）
- 計画一覧メニューで登録済みブロックをテーブル表示（キーワード/承認状態/日時範囲で検索・横/縦スクロール・列幅調整・長さは時間表示・編集ダイアログ起動）
- 在庫データ一覧で品目名の横に単位を表示（グリッドセル内の単位表示は省略）
- 計画データの JSON エクスポート
...
- `VITE_EXPOSE_LOCAL_NETWORK=true` を設定すると、開発/プレビューサーバーを `0.0.0.0` で起動しローカルネットワーク公開できます（未設定時は `127.0.0.1` のみ）。
...
Excel取り込みタブで `.xlsx` / `.xls` / `.csv` を選択し、「取り込み」ボタンを押すと日別在庫・品目マスタ・原料マスタを更新できます。読み込み対象は **先頭シート** のみで、1行目をヘッダーとして判定します。
ヘッダー名が異なる場合は、ヘッダー指定欄に候補名を入力して「設定を保存」を押すことで、次回以降も同じ候補で判定できます。
各カード右上の「CSVエクスポート」から、取り込み対象のテーブルをCSVで出力できます。
...
日別在庫は日付と品目コードをキーに既存データを更新し、存在しない組み合わせは追加されます。
...
Excel取り込みで更新する日別在庫の保存先です。日付と品目コードをキーに既存行を上書きし、未登録の組み合わせは追加されます。
`entries` には在庫数に加えて出荷数（`shipped`）も含めます。

| メソッド | 説明 | 入出力 |
| --- | --- | --- |
| GET | 日別在庫を取得 | `updatedAtISO` と `entries` を返却 |
| POST | 日別在庫を保存 | `entries` を受け取り日付 + 品目コードで更新/追加 |
